...
if __name__ == '__main__':
    ...
    # result = misp.search(publish_timestamp=args.last, limit=args.limit, page=args.page, pythonify=True, type=["ip-src", "ip-dst", "url"])
    result = (misp.search(controller='attributes', type_attribute=['ip-src','ip-dst','url'] ,publish_timestamp=args.last, limit=args.limit, page=args.page, return_format='json', Pythonify=False))
    if not result:
        print('No results for that time period')
        exit(0)

    #df=pd.read_csv(result)
    df=pd.DataFrame.from_records(result["Attribute"])
    #print(df.columns)
    df = df.join(json_normalize(df["Event"].tolist()).add_prefix("Event.")).drop(["Event"], axis=1)
    #info_df.set_index(df.index, inplace=True)
    #df.join(info_df)
    #del df.drop(df.columns[2], inplace=True)
    print(df.head(5))
    ##info = df.pop('Event').str.findall(r".*info': '(.+?)'")
    #info = df.pop('Event').str.findall('*fo.*')
    #print(info)
    #v = df.pop('Event')
    #print(v[3])
    #dt=df.explode('Event')
